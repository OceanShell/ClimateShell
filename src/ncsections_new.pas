...
uses
  ...
  Buttons, ComCtrls, ExtCtrls, Spin, IniFiles, BufDataSet, DB, Variants, Math;

type

  { Tfrmsections_new }

  Tfrmsections_new = class(TForm)
    btnGoogle: TButton;
    btnOpenBLN: TButton;
...
var
  frmsections_new: Tfrmsections_new;
...
function Tfrmsections_new.FilterRecordCount(dataset : TDataset) : integer;
begin
  try
    result := 0;
...
procedure Tfrmsections_new.FormShow(Sender: TObject);
Var
  Ini:TIniFile;
begin
...
procedure Tfrmsections_new.cbLvl1DropDown(Sender: TObject);
Var
 fdb:TSearchRec;
begin
...
procedure Tfrmsections_new.cbClr1DropDown(Sender: TObject);
Var
 fdb:TSearchRec;
begin
...
procedure Tfrmsections_new.btnPlotClick(Sender: TObject);
Var
  k, k_f, k_d:integer;
begin
...
procedure Tfrmsections_new.RunSection;
Var
  Ini: TIniFile;
...
procedure Tfrmsections_new.GetSection(fname, par: string; g_max:real);
Var
  Ini:TIniFile;
  ll, lt_i, ln_i, a, pp:integer;
  status, ncid, varidp, ndimsp, varnattsp, dimid:integer;
...
begin
 ...
 try
   ...
   if ncLev_arr[ll]<=g_max then begin
    inc(ll_max);
    AssignFile(out_f, SectionPath+'tmp'+PathDelim+inttostr(ll)+'.tmp'); rewrite(out_f);
   ...
   //  showmessage(floattostr(lat1)+'  '+floattostr(lon1));
      ...
      if (val0<>missing[0]) and (val0<>-9999) and (val0<>0) then begin
...
procedure Tfrmsections_new.rgAutomationSelectionChanged(Sender: TObject);
begin
 if rgAutomation.ItemIndex=0 then
    TabSheet2.Caption:=SAutomationOFF else
    TabSheet2.Caption:=SAutomationON;
end;


procedure Tfrmsections_new.btnSettingsClick(Sender: TObject);
begin
...
procedure Tfrmsections_new.btnOpenFolderClick(Sender: TObject);
begin
   OpenDocument(PChar(SectionPath));
end;


procedure Tfrmsections_new.btnOpenScriptClick(Sender: TObject);
Var
...
procedure Tfrmsections_new.btnOpenBLNClick(Sender: TObject);
begin
...
procedure Tfrmsections_new.btnGoogleClick(Sender: TObject);
Var
Ini:TIniFile;
f_out:text;
...
procedure Tfrmsections_new.btnSaveSectionClick(Sender: TObject);
Var
  dat: text;
  k: integer;
...
procedure Tfrmsections_new.FormClose(Sender: TObject;
