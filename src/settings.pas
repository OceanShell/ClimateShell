...
begin
 ...
 PythonDefault :=GlobalPath+'Python27'+PathDelim+'python.exe';
  ...
  try
   ...
   eRelErr.Text           :=Ini.ReadString  ( 'main', 'RelativeError',    '0.25');
   chkRelErr.Checked      :=Ini.ReadBool    ( 'main', 'UseRelativeError', true);
...
begin
  if frmmain.SDD.Execute then eSupportPath.Text:= frmmain.SDD.FileName;
end;
...
begin
  frmmain.OD.Filter:='Python.exe|Python.exe';
  if frmmain.OD.Execute then ePythonPath.Text:= frmmain.OD.FileName;
end;
...
memo2.Clear;
 Ini := TIniFile.Create(IniFileName);
  ...
  frmmain.RunScript(1, '-m pip install scipy',      memo2);
  frmmain.RunScript(1, '-m pip install pandas',     memo2);
  frmmain.RunScript(1, '-m pip install matplotlib', memo2);
  frmmain.RunScript(1, '-m pip install motuclient', memo2);
  frmmain.RunScript(1, '-m pip install cdsapi',     memo2);
end;
...
begin
 Ini := TIniFile.Create(IniFileName);
  try
   ...
   Ini.WriteString ( 'main', 'PythonPath',       ePythonPath.Text);
   Ini.WriteString ( 'main', 'RelativeError',    eRelErr.Text);
   Ini.WriteBool   ( 'main', 'UseRelativeError', chkRelErr.Checked);
