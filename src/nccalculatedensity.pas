...
type
  ...
  private
    procedure CalculateDensity(fname: string);
  public

  end;

var
  frmdensity: Tfrmdensity;
...
procedure Tfrmdensity.FormShow(Sender: TObject);
begin
...
procedure Tfrmdensity.btnCalculateClick(Sender: TObject);
Var
  k:integer;
begin
  ...
  frmmain.ProgressBar1.Max:=frmmain.cbFiles.count;
   for k:=0 to frmmain.cbFiles.count-1 do begin
    ...
    CalculateDensity(ncname); // else break;
...
procedure Tfrmdensity.cbVariableTSelect(Sender: TObject);
begin
...
procedure Tfrmdensity.CalculateDensity(fname: string);
Var
...
begin
 try
  ...
  start:=GetMemory(SizeOf(TArraySize_t)*4); // get memory for start pointer
    ...
    if (val0t<>missingT[0]) and (val0T<>-9999) and
       (val0s<>missingT[0]) and (val0s<>-9999) then begin
          ...
          if rgAlgorithm.ItemIndex=0 then begin
            p:=10.1325;
            SA  := gsw_SA_from_SP(val1s, p, nclon_arr[ln_i], nclat_arr[lt_i]);
            dens:= gsw_rho_t_exact(sa, val1t, p)-1000;
          end;

          if rgAlgorithm.ItemIndex=1 then begin
            p:=0;
  ...
  units         := 'kg m-3';
  unit_long     := 'Kilogram per cubic meter' ;

  if rgAlgorithm.ItemIndex=0 then alg_name:='TEOS-10' else alg_name:='IOS-80';
